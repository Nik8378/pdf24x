    ...
    <div className="space-y-2">
      <button
        ...
        className={`w-full flex items-center justify-center gap-2 font-semibold text-[14px] py-3 rounded-full transition-all ${
          disabled
            ? "bg-[var(--hover-soft)] border border-[var(--line)] text-[var(--txt-2)] cursor-not-allowed rounded-full"
            : "bg-[var(--inv-bg)] text-[var(--inv-txt)] hover:bg-[var(--inv-bg)] active:scale-[0.99] shadow-md hover:shadow-lg rounded-full"
        }`}
      >
        <FileDown size={16} strokeWidth={2} />
        {disabled ? "Add images to convert" : `Convert ${images.length > 0 ? images.length + " images" : ""} to PDF`}
      </button>
      <p className="text-center text-[11.5px] text-[var(--txt-2)]">
...
export function ProgressOverlay() {
  ...
  return (
    <div className="fixed inset-0 bg-[var(--surface)]/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl shadow-xl p-8 max-w-sm w-full text-center">
        {/* Spinner */}
        ...
        <p className="text-[15px] font-semibold text-[var(--txt)] mb-1">{progress.label}</p>
        <p className="text-[12.5px] text-[var(--txt-2)] mb-5">{progress.sub}</p>

        {/* Progress bar */}
        <div className="bg-[var(--hover-soft)] rounded-full h-1.5 overflow-hidden">
          <div
            className="h-full bg-accent rounded-full transition-all duration-300"
            style={{ width: `${progress.percent}%` }}
          />
        </div>
        <p className="text-[11px] text-[var(--txt-2)] mt-2">{progress.percent}%</p>
      </div>
    </div>
  );
}
