    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5 pb-24 lg:pb-8">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--txt)] mb-1">CSS Minifier & Formatter</h1>
            <p className="text-[13px] text-[var(--txt-2)]">Minify CSS to reduce file size or format/beautify CSS for readability. Removes comments, whitespace, and redundant characters.</p>
          </div>

          <div className="flex flex-wrap items-center gap-3 mb-4">
            <div className="flex bg-[var(--hover-soft)] rounded-full p-1">
              <button onClick={() => handleMode("minify")} className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-[13px] font-semibold transition-all ${mode === "minify" ? "bg-accent text-white shadow" : "text-[var(--txt-2)] hover:text-[var(--txt)]"}`}><Minimize2 size={13} /> Minify</button>
              <button onClick={() => handleMode("format")} className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-[13px] font-semibold transition-all ${mode === "format" ? "bg-accent text-white shadow" : "text-[var(--txt-2)] hover:text-[var(--txt)]"}`}><Maximize2 size={13} /> Format</button>
            </div>
            <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 bg-[var(--hover-soft)] hover:bg-[var(--hover-soft)] text-[var(--txt-2)] font-medium text-[13px] px-4 py-2 rounded-full transition-all"><Upload size={14} /> Upload .css</button>
            <input ref={fileRef} type="file" accept=".css,text/css" className="hidden" onChange={e => e.target.files?.[0] && handleFile(e.target.files[0])} />
            <button onClick={() => { setInput(SAMPLE); process(SAMPLE, mode); }} className="bg-[var(--hover-soft)] hover:bg-[var(--hover-soft)] text-[var(--txt-2)] font-medium text-[13px] px-4 py-2 rounded-full transition-all">Sample CSS</button>
          ...
          {savings && (
            <div className="flex flex-wrap items-center gap-4 px-4 py-3 bg-green-50 border border-green-100 rounded-xl mb-4">
              ...
              <span className="text-[11.5px] text-[var(--txt-2)]">Original: <strong className="text-[var(--txt)]">{savings.original} B</strong></span>
              <span className="text-[11.5px] text-[var(--txt-2)]">Minified: <strong className="text-[var(--txt)]">{savings.minified} B</strong></span>
              <span className="text-[11.5px] text-[var(--txt-2)]">Saved: <strong className="text-green-600">{savings.original - savings.minified} B</strong></span>
              <div className="ml-auto w-32 h-2 bg-[var(--hover-soft)] rounded-full overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-[12px] font-bold text-[var(--txt)] uppercase tracking-widest">Input CSS</span>
              ...
              <textarea value={input} onChange={e => handleInput(e.target.value)} placeholder="Paste your CSS here..." spellCheck={false}
                className="w-full min-h-[500px] bg-[var(--inv-bg)] text-[var(--inv-txt)] font-mono text-[13px] rounded-2xl p-4 border border-[var(--line-mid)] focus:outline-none focus:ring-2 focus:ring-accent resize-none leading-relaxed" />
            </div>
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-[12px] font-bold text-[var(--txt)] uppercase tracking-widest">Output</span>
                <div className="flex gap-2">
                  {output && <button onClick={handleDownload} className="text-[11px] text-[var(--txt-2)] hover:text-accent flex items-center gap-1"><Download size={12} />Save</button>}
              ...
              <textarea readOnly value={output} placeholder="Output appears here..." className="w-full min-h-[500px] bg-[var(--surface)] border border-[var(--line)] text-[var(--txt)] font-mono text-[13px] rounded-2xl p-4 focus:outline-none resize-none leading-relaxed" />
            </div>
          </div>
        </main>
      </div>
