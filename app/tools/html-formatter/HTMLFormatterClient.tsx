    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5 pb-24 lg:pb-8">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--txt)] mb-1">HTML Formatter & Minifier</h1>
            <p className="text-[13px] text-[var(--txt-2)]">Beautify or minify HTML code instantly. Clean indentation, remove whitespace, format tags properly.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            {mode === "format" && (
              <div className="flex items-center gap-2">
                ...
                {[2, 4].map(n => <button key={n} onClick={() => handleIndent(n)} className={`w-8 h-8 rounded-lg text-[12px] font-bold transition-all ${indent === n ? "bg-accent text-white" : "bg-[var(--hover-soft)] text-[var(--txt-2)] hover:bg-[var(--hover-soft)]"}`}>{n}</button>)}
              </div>
            )}
            <button onClick={() => { setInput(SAMPLE); process(SAMPLE, mode, indent); }} className="bg-[var(--hover-soft)] hover:bg-[var(--hover-soft)] text-[var(--txt-2)] font-medium text-[13px] px-4 py-2 rounded-full transition-all">Sample HTML</button>
          ...
          {stats && (
            <div className="flex items-center gap-4 px-4 py-2.5 bg-green-50 border border-green-100 rounded-xl mb-4">
              ...
              <span className="text-[11.5px] text-[var(--txt-2)] ml-auto">Size: <strong className="text-[var(--txt)]">{stats.size}</strong></span>
              <span className="text-[11.5px] text-[var(--txt-2)]">Lines: <strong className="text-[var(--txt)]">{stats.lines}</strong></span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-[12px] font-bold text-[var(--txt)] uppercase tracking-widest">Input HTML</span>
            ...
            <div>
              ...
              <textarea readOnly value={output} placeholder="Formatted HTML appears here..." className="w-full min-h-[500px] bg-[var(--surface)] border border-[var(--line)] text-[var(--txt)] font-mono text-[13px] rounded-2xl p-4 focus:outline-none resize-none leading-relaxed" />
            </div>
          </div>
        </main>
      </div>
