    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5 pb-24 lg:pb-8">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--txt)] mb-1">Base64 Encoder / Decoder</h1>
            <p className="text-[13px] text-[var(--txt-2)]">Encode text or images to Base64, or decode Base64 strings back to text. Instant, free, browser-based.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <div className="flex bg-[var(--hover-soft)] rounded-full p-1">
              {(["encode","decode"] as Mode[]).map(m => (
                <button key={m} onClick={() => handleMode(m)}
                  className={`px-5 py-2 rounded-full text-[13px] font-semibold capitalize transition-all ${mode === m ? "bg-accent text-white shadow" : "text-[var(--txt-2)] hover:text-[var(--txt)]"}`}>
                  {m}
                </button>
              ))}
            </div>
            <div className="flex bg-[var(--hover-soft)] rounded-full p-1">
              {(["text","image"] as InputType[]).map(t => (
                <button key={t} onClick={() => setInputType(t)}
                  className={`px-4 py-2 rounded-full text-[13px] font-medium capitalize transition-all ${inputType === t ? "bg-[var(--surface)] shadow text-accent" : "text-[var(--txt-2)] hover:text-[var(--txt)]"}`}>
                  {t}
                </button>
              ))}
            </div>
            <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 bg-[var(--hover-soft)] hover:bg-[var(--hover-soft)] text-[var(--txt-2)] font-medium text-[13px] px-4 py-2 rounded-full transition-all">
            ...
            <button onClick={() => { const s = mode === "encode" ? SAMPLE_TEXT : SAMPLE_B64; setInput(s); process(s, mode); }} className="flex items-center gap-1.5 bg-[var(--hover-soft)] hover:bg-[var(--hover-soft)] text-[var(--txt-2)] font-medium text-[13px] px-4 py-2 rounded-full transition-all">
              Sample
            </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-4 items-start">
            ...
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-[12px] font-bold text-[var(--txt)] uppercase tracking-widest">{mode === "encode" ? "Plain Text / Image" : "Base64 String"}</span>
                <span className="text-[11px] text-[var(--txt-2)]">{input.length} chars</span>
              </div>
              {imagePreview && inputType === "image" ? (
                <div className="bg-[var(--hover-soft)] rounded-2xl p-4 min-h-[300px] flex items-center justify-center">
                ...
                <textarea value={input} onChange={e => handleInput(e.target.value)} placeholder={mode === "encode" ? "Enter text to encode..." : "Enter Base64 string to decode..."}
                  spellCheck={false} className="w-full min-h-[300px] bg-[var(--inv-bg)] text-[var(--inv-txt)] font-mono text-[13px] rounded-2xl p-4 border border-[var(--line-mid)] focus:outline-none focus:ring-2 focus:ring-accent resize-none leading-relaxed" />
              )}
            </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-[12px] font-bold text-[var(--txt)] uppercase tracking-widest">{mode === "encode" ? "Base64 Output" : "Decoded Text"}</span>
              ...
              {error ? (
                ...
                <textarea readOnly value={output} placeholder="Output appears here..."
                  className="w-full min-h-[300px] bg-[var(--surface)] border border-[var(--line)] text-[var(--txt)] font-mono text-[13px] rounded-2xl p-4 focus:outline-none resize-none leading-relaxed" />
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
            <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4">
              <h2 className="text-[13px] font-bold text-[var(--txt)] mb-2">What is Base64?</h2>
              <p className="text-[12.5px] text-[var(--txt-2)] leading-relaxed">Base64 is a binary-to-text encoding scheme that converts binary data into ASCII characters. Commonly used in email, data URLs, and API authentication.</p>
            </div>
            <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4">
              <h2 className="text-[13px] font-bold text-[var(--txt)] mb-2">Common Use Cases</h2>
              <ul className="text-[12.5px] text-[var(--txt-2)] space-y-1">
