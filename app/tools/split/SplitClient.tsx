    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--txt)]">Split PDF</h1>
            <p className="text-[13px] text-[var(--txt-2)]">Split PDF pages into multiple PDFs. Extract specific pages or split by range.</p>
          </div>
          ...
          <div className="flex flex-col xl:flex-row gap-4 items-start">
            <div className="flex-1 min-w-0 w-full space-y-4">
              ...
              <div onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop}
                ...
                className={`border-2 border-dashed rounded-2xl p-7 text-center cursor-pointer transition-all shadow-sm flex flex-col items-center justify-center min-h-[200px] ${dragging ? "border-accent bg-accent-light" : "border-[var(--line-mid)] bg-[var(--surface)] hover:border-accent hover:bg-accent-light"}`}>
                ...
                <div className="w-11 h-11 rounded-full bg-[var(--hover-soft)] flex items-center justify-center mb-3">
                  <UploadCloud size={20} className="text-[var(--txt-2)]" />
                </div>
                <p className="text-[14px] font-semibold text-[var(--txt)] mb-1">Drag & drop your PDF here</p>
                <p className="text-[12.5px] text-[var(--txt-2)] mb-4">or click to browse file</p>
                ...
                <p className="mt-3 text-[11.5px] text-[var(--txt-2)]">Max file size: 100MB</p>
              </div>

              {/* File info */}
              {pdf && (
                <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl px-4 py-3 flex items-center gap-3 shadow-sm">
                  <div className="w-10 h-10 bg-red-50 rounded-lg flex items-center justify-center shrink-0">
                    <FileText size={18} className="text-red-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-[var(--txt)] truncate">{pdf.name}</p>
                    <p className="text-[11.5px] text-[var(--txt-2)]">{fmtBytes(pdf.size)} • {pdf.pages} pages</p>
                  </div>
                  <button onClick={() => setPdf(null)} className="p-2 rounded-lg text-[var(--txt-2)] hover:text-red-500 hover:bg-red-50"><Trash2 size={14} /></button>
                </div>
              )}
              ...
              {pdf && (
                <>
                  <div>
                    <p className="text-[14px] font-bold text-[var(--txt)] mb-3">Choose Split Method</p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {methodCards.map(m => (
                        <button key={m.id} onClick={() => setMethod(m.id)}
                          className={`text-left p-3 rounded-xl border transition-all ${method === m.id ? "border-accent bg-accent-light" : "border-[var(--line)] bg-[var(--surface)] hover:border-accent/40"}`}>
                          <div className="flex items-center gap-2 mb-1">
                            <Scissors size={14} className={method === m.id ? "text-accent" : "text-[var(--txt-2)]"} />
                            <p className="text-[13px] font-bold text-[var(--txt)]">{m.label}</p>
                          </div>
                          <p className="text-[11.5px] text-[var(--txt-2)]">{m.desc}</p>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {method === "extract" && (
                    <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4 shadow-sm">
                      <p className="text-[13px] font-bold text-[var(--txt)] mb-1">Extract Pages</p>
                      <p className="text-[12px] text-[var(--txt-2)] mb-3">Enter page numbers you want to extract (e.g., 1, 3, 5-8, 10)</p>
                      <input value={pageInput} onChange={e => setPageInput(e.target.value)}
                        className="w-full bg-[var(--hover-soft)] border border-[var(--line)] rounded-lg px-3 py-2.5 text-[13px] text-[var(--txt)] focus:outline-none focus:border-accent mb-3"
                        placeholder="1, 3, 5-8, 10-12" />
                      <div className="flex items-center justify-between text-[12.5px]">
                        <span className="text-[var(--txt-2)]">Total pages: {pdf.pages}</span>
                      ...
                      {selectedPages.length > 0 && (
                        <div className="mt-2 flex items-start gap-1.5 text-[11.5px] text-[var(--txt-2)] bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                  ...
                  {method === "range" && (
                    <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4 shadow-sm">
                      <p className="text-[13px] font-bold text-[var(--txt)] mb-1">Split by Range</p>
                      <p className="text-[12px] text-[var(--txt-2)] mb-3">Enter ranges separated by commas. Each range becomes a separate PDF.</p>
                      <input value={rangeInput} onChange={e => setRangeInput(e.target.value)}
                        className="w-full bg-[var(--hover-soft)] border border-[var(--line)] rounded-lg px-3 py-2.5 text-[13px] text-[var(--txt)] focus:outline-none focus:border-accent"
                        placeholder="1-5, 6-10, 11-20" />
                    </div>
                  )}

                  {method === "every" && (
                    <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4 shadow-sm flex items-start gap-2">
                      <Info size={14} className="text-accent shrink-0 mt-0.5" />
                      <p className="text-[12.5px] text-[var(--txt-2)]">All {pdf.pages} pages will be split into {pdf.pages} individual PDF files and downloaded one by one.</p>
                    </div>
                  )}
                </>
              )}
            ...
            <div className="w-full xl:w-[280px] shrink-0 xl:sticky xl:top-14">
              <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-[var(--line)]">
                  <Scissors size={14} className="text-[var(--txt-2)]" />
                  <h3 className="text-[13px] font-bold text-[var(--txt)]">Split Settings</h3>
                </div>
                <div className="space-y-3.5">
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">File Name</p>
                    <input value={fileName} onChange={e => setFileName(e.target.value)}
                      className="w-full bg-[var(--hover-soft)] border border-[var(--line)] rounded-lg px-3 py-2 text-[13px] text-[var(--txt)] focus:outline-none focus:border-accent"
                      placeholder="Split-Document" />
                    <p className="text-[11px] text-[var(--txt-2)] mt-1">Output file names will be based on this.</p>
                  </div>
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Output Options</p>
                    {[["multiple", "Multiple PDF files", "Save each split as a separate PDF file."],["zip", "Zip all files", "Compress all split PDFs into a ZIP file."]].map(([val, label, desc]) => (
                      <label key={val} className="flex items-start gap-2 mb-2 cursor-pointer" onClick={() => setOutputOption(val as OutputOption)}>
                        <div className={`w-4 h-4 mt-0.5 rounded-full border-2 flex items-center justify-center shrink-0 ${outputOption === val ? "border-accent bg-accent" : "border-[var(--line-mid)]"}`}>
                          {outputOption === val && <div className="w-1.5 h-1.5 rounded-full bg-[var(--surface)]" />}
                        </div>
                        <div><p className="text-[12.5px] font-medium text-[var(--txt)]">{label}</p><p className="text-[11px] text-[var(--txt-2)]">{desc}</p></div>
                      </label>
                    ))}
                  </div>
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Page Label</p>
                    <label className="flex items-center gap-2 cursor-pointer" onClick={() => setAddPageNumbers(!addPageNumbers)}>
                      <div className={`w-4 h-4 rounded border-2 flex items-center justify-center ${addPageNumbers ? "bg-accent border-accent" : "border-[var(--line-mid)]"}`}>
                      ...
                      <span className="text-[12.5px] text-[var(--txt)]">Add page numbers</span>
                    </label>
                    <p className="text-[11px] text-[var(--txt-2)] ml-6">Add page numbers to the extracted PDF.</p>
                  </div>
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Advanced Options</p>
                    {([["keepBookmarks", keepBookmarks, setKeepBookmarks, "Keep bookmarks"],["keepLinks", keepLinks, setKeepLinks, "Keep links"],["keepFormFields", keepFormFields, setKeepFormFields, "Keep form fields"]] as [string, boolean, React.Dispatch<React.SetStateAction<boolean>>, string][]).map(([key, val, setter, label]) => (
                      <label key={key} className="flex items-center gap-2 mb-2 cursor-pointer" onClick={() => setter(!val)}>
                        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center ${val ? "bg-accent border-accent" : "border-[var(--line-mid)]"}`}>
                ...
                <div className="border-t border-[var(--line)] mt-3 pt-3">
                  <button onClick={handleSplit} disabled={!pdf || loading}
                    className={`w-full flex items-center justify-center gap-2 font-semibold text-[14px] py-3 rounded-full transition-all ${!pdf || loading ? "bg-[var(--hover-soft)] text-[var(--txt-2)] cursor-not-allowed" : "bg-accent hover:bg-accent-dark text-white shadow-md"}`}>
                    {loading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full spin" /> : <Scissors size={16} />}
                    {loading ? "Splitting…" : "Split PDF →"}
                  </button>
                  <p className="text-center text-[11px] text-[var(--txt-2)] mt-2">Your files are processed securely and never stored.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mx-4 sm:mx-6 lg:mx-8 mt-6 mb-6 space-y-4">
        <div className="rounded-2xl border border-[var(--line-strong)] bg-[var(--surface)] p-6" style={{ boxShadow: "3px 3px 0 0 var(--line-strong)" }}>
          ...
          <ol className="space-y-3">
            <li className="flex gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: "#FF6B5E" }}>1</span><div><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Upload your PDF</p><p className="text-xs mt-0.5" style={{ color: "var(--txt-2)" }}>Drop your PDF file into the upload area.</p></div></li>
            <li className="flex gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: "#FF6B5E" }}>2</span><div><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Select split method</p><p className="text-xs mt-0.5" style={{ color: "var(--txt-2)" }}>Choose to split by page range, extract specific pages, or split every N pages.</p></div></li>
            <li className="flex gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: "#FF6B5E" }}>3</span><div><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Download split files</p><p className="text-xs mt-0.5" style={{ color: "var(--txt-2)" }}>Download individual pages or a ZIP file containing all split documents.</p></div></li>
          </ol>
        </div>
        <div className="rounded-2xl border border-[var(--line-strong)] bg-[var(--surface)] p-6" style={{ boxShadow: "3px 3px 0 0 var(--line-strong)" }}>
          <h2 className="text-lg font-extrabold mb-4" style={{ color: "var(--txt)", fontFamily: "Archivo, Inter, sans-serif" }}>Frequently Asked Questions</h2>
          <div className="space-y-4">
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Can I extract specific pages?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>Yes. Select individual pages visually or enter custom page ranges like 1-3,5,8-10.</p></div>
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>What split modes are available?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>You can split by custom ranges, select specific pages visually, or split into equal chunks of N pages each.</p></div>
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Will the output be one file or multiple?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>Depends on your split — a single range produces one PDF, multiple ranges produce a ZIP with multiple PDFs.</p></div>
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Is the original PDF affected?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>No. The original file is never modified. Split files are new documents.</p></div>
          </div>
        </div>
      </div>
