...
function SortableRow({ item, onRemove }: { item: PDFItem; onRemove: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-[var(--surface)] border border-[var(--line)] rounded-xl px-3 py-2.5 flex items-center gap-3 hover:border-[var(--line-mid)] transition-all">
      <div {...attributes} {...listeners} className="text-[var(--txt-2)] cursor-grab p-0.5"><GripVertical size={15} /></div>
      <div className="w-9 h-9 bg-red-50 rounded-lg flex items-center justify-center shrink-0">
        <FileText size={16} className="text-red-500" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-semibold text-[var(--txt)] truncate">{item.name}</p>
        <p className="text-[11.5px] text-[var(--txt-2)]">{fmtBytes(item.size)}{item.pages ? ` • ${item.pages} pages` : ""}</p>
      </div>
      <p className="text-[12px] text-[var(--txt-2)] hidden sm:block">{fmtBytes(item.size)}</p>
      <button onClick={onRemove} className="p-1.5 rounded-lg text-[var(--txt-2)] hover:text-red-500 hover:bg-red-50 transition-all"><Trash2 size={14} /></button>
    </div>
  );
}
    ...
    <div className="flex border border-[var(--line)] rounded-lg overflow-hidden">
      {options.map(o => (
        <button key={o} onClick={() => onChange(o)}
          className={`flex-1 py-1.5 text-[12px] font-medium transition-colors border-r border-[var(--line)] last:border-0 ${value === o ? "bg-txt text-white" : "bg-[var(--surface)] text-[var(--txt-2)] hover:bg-[var(--hover-soft)]"}`}>
          {o}
        </button>
      ))}
    </div>
    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--txt)]">Merge PDF</h1>
            <p className="text-[13px] text-[var(--txt-2)]">Combine multiple PDF files into a single PDF. Easy, fast and secure.</p>
          </div>
          ...
          <div className="flex flex-col xl:flex-row gap-4 items-start">
            <div className="flex-1 min-w-0 w-full space-y-4">
              <div onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop}
                ...
                <p className="text-[14px] font-semibold text-[var(--txt)] mb-1">Drag & drop your PDF files here</p>
                <p className="text-[12.5px] text-[var(--txt-2)] mb-4">or click to browse files</p>
                ...
                <p className="mt-3 text-[11.5px] text-[var(--txt-2)]">No file size limit • Multiple files supported</p>
              </div>

              {files.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[13.5px] font-semibold text-[var(--txt)]">Your PDFs ({files.length})</p>
            ...
            <div className="w-full xl:w-[280px] shrink-0 xl:sticky xl:top-14">
              <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4 shadow-sm overflow-y-auto">
                <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-[var(--line)]">
                  <Link2 size={14} className="text-[var(--txt-2)]" />
                  <h3 className="text-[13px] font-bold text-[var(--txt)]">Merge Settings</h3>
                </div>

                <div className="space-y-3.5">
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Page Size</p>
                    <select value={pageSize} onChange={e => setPageSize(e.target.value)}
                      className="w-full bg-[var(--hover-soft)] border border-[var(--line)] rounded-lg px-3 py-2 text-[13px] text-[var(--txt)] cursor-pointer appearance-none focus:outline-none focus:border-accent"
                  ...
                  <div><p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Page Orientation</p><BtnGroup options={["Auto","Portrait","Landscape"]} value={orientation} onChange={setOrientation} /></div>
                  <div><p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Margins</p><BtnGroup options={["None","Small","Medium","Large"]} value={margins} onChange={setMargins} /></div>

                  <div className="flex items-center justify-between">
                    <div><p className="text-[12.5px] font-medium text-[var(--txt)]">Add Blank Page Between Files</p><p className="text-[11px] text-[var(--txt-2)]">Adds a blank page between each PDF</p></div>
                    <button onClick={() => setBlankBetween(!blankBetween)} className={`w-9 h-5 rounded-full transition-all relative ${blankBetween ? "bg-accent" : "bg-[var(--hover-soft)]"}`}>
                      <div className={`absolute top-0.5 w-4 h-4 bg-[var(--surface)] rounded-full shadow transition-all ${blankBetween ? "left-4" : "left-0.5"}`} />
                    </button>
                  </div>

                  {blankBetween && <div><p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Blank Page Position</p><BtnGroup options={["Start","Between","End"]} value={blankPos.charAt(0).toUpperCase()+blankPos.slice(1)} onChange={v => setBlankPos(v.toLowerCase())} /></div>}

                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Output File Name</p>
                  ...
                  <div>
                    ...
                    {([["addTOC", addTOC, setAddTOC, "Add Table of Contents"],["addPageNumbers", addPageNumbers, setAddPageNumbers, "Add Page Numbers"],["bookmark", bookmark, setBookmark, "Bookmark each file"]] as [string, boolean, React.Dispatch<React.SetStateAction<boolean>>, string][]).map(([key, val, setter, label]) => (
                      <label key={key} className="flex items-center gap-2 mb-2 cursor-pointer">
                        <div onClick={() => setter(!val)} className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all ${val ? "bg-accent border-accent" : "border-[var(--line-mid)]"}`}>
                ...
                <div className="border-t border-[var(--line)] mt-3 pt-3">
                  <button onClick={handleMerge} disabled={files.length < 2 || loading}
                    className={`w-full flex items-center justify-center gap-2 font-semibold text-[14px] py-3 rounded-full transition-all ${files.length < 2 || loading ? "bg-[var(--hover-soft)] text-[var(--txt-2)] cursor-not-allowed" : "bg-accent hover:bg-accent-dark text-white shadow-md"}`}>
      ...
      <div className="mx-4 sm:mx-6 lg:mx-8 mt-6 mb-6 space-y-4">
        ...
        <div className="rounded-2xl border border-[var(--line-strong)] bg-[var(--surface)] p-6" style={{ boxShadow: "3px 3px 0 0 var(--line-strong)" }}>
          ...
          <div className="space-y-4">
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>How many PDFs can I merge?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>You can merge as many PDFs as you need in a single operation.</p></div>
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Does merging preserve formatting?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>Yes. Merging combines PDF pages without altering any content, fonts, images, or formatting.</p></div>
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Can I reorder pages before merging?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>Yes. Drag the files into the order you want before clicking Merge.</p></div>
            <div className="border-b pb-4 last:border-0 last:pb-0" style={{ borderColor: "#e5e7eb" }}><p className="text-sm font-semibold" style={{ color: "var(--txt)" }}>Is there a file size limit?</p><p className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--txt-2)" }}>Individual files can be up to 100MB each.</p></div>
          </div>
        </div>
      </div>
