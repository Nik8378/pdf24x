    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--txt)]">PDF to JPG</h1>
            <p className="text-[13px] text-[var(--txt-2)]">Convert PDF pages to high-quality JPG images. Free and private.</p>
          </div>
              ...
              {pdf && (
                <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl px-4 py-3 flex items-center gap-3 shadow-sm">
                  ...
                  <button onClick={() => { setPdf(null); setPreviews([]); }}
                    className="p-2 rounded-lg text-[var(--txt-2)] hover:text-red-500 hover:bg-red-50">
                    <Trash2 size={14} />
                  </button>
                </div>
              )}
              ...
              {loading && (
                <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[13px] font-semibold text-[var(--txt)]">Converting pages…</p>
              ...
              {previews.length > 0 && (
                <div className="bg-[var(--surface)] border border-[var(--line)] rounded-xl p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[13px] font-bold text-[var(--txt)]">Conversion Preview</p>
                    <span className="text-[11.5px] text-[var(--txt-2)]">{pdf?.pages} pages total</span>
                  </div>
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {previews.map((p) => (
                      <div key={p.page} className="border border-[var(--line)] rounded-lg overflow-hidden">
                        <img src={p.url} alt={`Page ${p.page}`} className="w-full object-cover" />
                        <p className="text-[10.5px] text-[var(--txt-2)] text-center py-1">p.{p.page}</p>
                      </div>
                    ))}
                  </div>
                  <div className="flex items-start gap-1.5 mt-3 text-[11.5px] text-[var(--txt-2)] bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
            ...
            <div className="w-full xl:w-[280px] shrink-0 xl:sticky xl:top-14">
              <div className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-[var(--line)]">
                  <Layers size={14} className="text-[var(--txt-2)]" />
                ...
                <div className="space-y-3.5">
                  {/* Quality */}
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Image Quality</p>
                    <select value={quality} onChange={(e) => setQuality(e.target.value)}
                      className="w-full bg-[var(--hover-soft)] border border-[var(--line)] rounded-lg px-3 py-2 text-[13px] text-[var(--txt)] appearance-none focus:outline-none focus:border-accent"
                    ...
                    <p className="text-[11px] text-[var(--txt-2)] mt-1">Higher quality = larger file size.</p>
                  </div>

                  {/* Resolution */}
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Resolution (DPI)</p>
                    <div className="flex border border-[var(--line)] rounded-lg overflow-hidden">
                      {["72", "96", "150", "300"].map((r) => (
                        <button key={r} onClick={() => setResolution(r)}
                          className={`flex-1 py-1.5 text-[12px] font-medium transition-colors border-r border-[var(--line)] last:border-0 ${resolution === r ? "bg-txt text-white" : "bg-[var(--surface)] text-[var(--txt-2)] hover:bg-[var(--hover-soft)]"}`}>
                          {r}
                        </button>
                      ))}
                    </div>
                    <p className="text-[11px] text-[var(--txt-2)] mt-1">150 = screen, 300 = print quality.</p>
                  </div>

                  {/* Page Range */}
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Page Range</p>
                    {[["all", "All Pages"], ["custom", "Custom Range"]].map(([val, label]) => (
                      <label key={val} className="flex items-center gap-2 mb-2 cursor-pointer"
                        ...
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${pageRange === val ? "border-accent bg-accent" : "border-[var(--line-mid)]"}`}>
                          {pageRange === val && <div className="w-1.5 h-1.5 rounded-full bg-[var(--surface)]" />}
                        </div>
                    ...
                    {pageRange === "custom" && (
                      <input value={customRange} onChange={(e) => setCustomRange(e.target.value)}
                        className="w-full bg-[var(--hover-soft)] border border-[var(--line)] rounded-lg px-3 py-2 text-[13px] text-[var(--txt)] focus:outline-none focus:border-accent mt-1"
                  ...
                  {/* Color Mode */}
                  <div>
                    <p className="text-[10.5px] font-bold text-[var(--txt)] uppercase tracking-widest opacity-50 mb-2">Color Mode</p>
                    <div className="flex border border-[var(--line)] rounded-lg overflow-hidden">
                      {["color", "grayscale"].map((m) => (
                        <button key={m} onClick={() => setColorMode(m)}
                          className={`flex-1 py-1.5 text-[12px] font-medium transition-colors border-r border-[var(--line)] last:border-0 capitalize ${colorMode === m ? "bg-txt text-white" : "bg-[var(--surface)] text-[var(--txt-2)] hover:bg-[var(--hover-soft)]"}`}>
                          {m}
                        </button>
                      ))}
                    </div>
                ...
                <div className="border-t border-[var(--line)] mt-3 pt-3">
                  ...
                  <p className="text-center text-[11px] text-[var(--txt-2)] mt-2">Files processed locally — never uploaded.</p>
                </div>
              </div>
            </div>
          </div>
