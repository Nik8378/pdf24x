    ...
    <>
      <div className="w-full flex gap-0 items-start">
        ...
        <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-5 pb-24 lg:pb-8">
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[#1a1917] mb-1">Excel to PDF Converter</h1>
          ...
          <div className="flex flex-col xl:flex-row gap-4 items-start">
            ...
            <div className="w-full xl:w-[300px] shrink-0 space-y-4">
              {!file ? (
                <div onDragOver={e => e.preventDefault()} onDrop={onDrop}
                  ...
                  <FileSpreadsheet size={32} className="text-[#7a7875] mx-auto mb-3" />
                  <p className="text-[13px] font-semibold text-[#1a1917] mb-1">Drop Excel file here</p>
                  <p className="text-[12px] text-[#7a7875] mb-3">Supports .xlsx, .xls, .csv</p>
                ...
                <div className="bg-white border border-[#1a1917]/10 rounded-2xl p-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-[#7a7875]">{fmtBytes(file.size)} · {sheets.length} sheet{sheets.length > 1 ? "s" : ""}</p>
                    </div>
                  ...
                  {sheets.length > 1 && (
                    <div>
                      <p className="text-[10.5px] font-bold text-[#7a7875] uppercase tracking-widest mb-1.5">Select Sheet</p>
                      <div className="flex flex-wrap gap-1.5">
                        {sheets.map((s, i) => (
                          <button key={i} onClick={() => setSelectedSheet(i)}
                            className={`px-2.5 py-1 rounded-lg text-[12px] font-medium transition-all border ${selectedSheet === i ? "bg-accent text-white border-accent" : "bg-[#f4f3f0] text-[#4a4845] border-[#e5e3de] hover:border-accent"}`}>
                            {s.name}
                          </button>
                        ))}
                      </div>
              ...
              <div className="bg-white border border-[#1a1917]/10 rounded-2xl p-4 space-y-3">
                <p className="text-[12px] font-bold text-[#1a1917] uppercase tracking-widest">PDF Options</p>

                <div>
                  <label className="text-[10.5px] font-bold text-[#7a7875] uppercase tracking-widest mb-1 block">Page Size</label>
                  <select value={options.pageSize}
                    onChange={e => setOptions(p => ({ ...p, pageSize: e.target.value }))}
                    className="w-full bg-[#f4f3f0] border border-[#e5e3de] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:border-accent">
                    {["A4", "A3", "Letter", "Legal"].map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>

                <div>
                  <label className="text-[10.5px] font-bold text-[#7a7875] uppercase tracking-widest mb-1 block">Orientation</label>
                  <div className="flex gap-2">
                    {(["portrait", "landscape"] as const).map(o => (
                      <button key={o} onClick={() => setOptions(p => ({ ...p, orientation: o }))}
                        className={`flex-1 py-2 rounded-lg text-[12px] font-medium capitalize border transition-all ${options.orientation === o ? "bg-accent text-white border-accent" : "bg-[#f4f3f0] text-[#4a4845] border-[#e5e3de] hover:border-accent/40"}`}>
                        {o}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-[10.5px] font-bold text-[#7a7875] uppercase tracking-widest">Font Size</label>
              ...
              <button onClick={handleConvert} disabled={!sheets.length || converting}
                className={`w-full flex items-center justify-center gap-2 font-semibold text-[14px] py-3.5 rounded-full transition-all shadow-md ${!sheets.length || converting ? "bg-[#f4f3f0] text-[#7a7875] cursor-not-allowed" : "bg-accent hover:bg-accent-dark text-white hover:shadow-lg"}`}>
                {converting
            ...
            <div className="flex-1 min-w-0 w-full">
              ...
              {loading && (
                <div className="bg-white border border-[#1a1917]/10 rounded-2xl p-8 text-center">
                  <div className="w-8 h-8 border-2 border-accent border-t-transparent rounded-full animate-spin mx-auto mb-3" />
                  <p className="text-[13px] text-[#7a7875]">Reading Excel file...</p>
                </div>
              )}

              {currentSheet && !loading && (
                <div className="bg-white border border-[#1a1917]/10 rounded-2xl overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[#e5e3de] bg-[#f4f3f0]">
                    <div className="flex items-center gap-2">
                      <CheckCircle size={15} className="text-green-600" />
                      <span className="text-[12px] font-bold text-[#1a1917]">{currentSheet.name}</span>
                      <span className="text-[11px] text-[#7a7875]">{currentSheet.rows} rows × {currentSheet.cols} cols</span>
                    </div>
                  </div>
                  <div className="overflow-auto max-h-[500px]">
                      ...
                      <tbody>
                        {currentSheet.data.slice(0, 100).map((row, ri) => (
                          <tr key={ri} className={ri === 0 ? "bg-[#1a1917] text-white" : ri % 2 === 0 ? "bg-white" : "bg-[#f9f9f9]"}>
                            <td className="px-2 py-1 text-[10px] text-[#7a7875] border-r border-[#e5e3de] w-8 text-center font-mono select-none">{ri + 1}</td>
                            {row.map((cell, ci) => (
                              <td key={ci} className={`px-3 py-1.5 border-r border-b border-[#e5e3de] whitespace-nowrap max-w-[200px] overflow-hidden text-ellipsis ${ri === 0 ? "font-bold" : ""}`}>
                                {cell}
                              </td>
                            ))}
                          </tr>
                    ...
                    {currentSheet.rows > 100 && (
                      <div className="px-4 py-3 text-[12px] text-[#7a7875] bg-[#f4f3f0] border-t border-[#e5e3de]">
              ...
              {!file && !loading && (
                <div className="bg-white border border-[#1a1917]/10 rounded-2xl p-12 text-center">
                  ...
                  <p className="text-[14px] font-semibold text-[#1a1917] mb-2">Upload an Excel file to preview</p>
                  <p className="text-[12.5px] text-[#7a7875]">Supports .xlsx, .xls and .csv files</p>
                </div>
              )}
            </div>
          </div>
